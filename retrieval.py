...
def retrieve_relevant_chunks(query, n_results=5, source_types=None, date_after=None, date_before=None):
    ...
    try:
        results = collection.query(query_texts=[query], n_results=n_results, where=where if where else None, include=['documents', 'metadatas', 'distances'])
    except Exception:
        results = collection.query(query_texts=[query], n_results=n_results, where=where if where else None, include=['documents', 'metadatas'])
    ...
    if any(k in query.lower() for k in ['widget', 'product', 'sku', 'code', 'refund', 'return', 'cancellation', 'policy', 'price', 'pricing', '30 days', 'window', 'sender', 'subject', 'from', 'date', 'body']):
