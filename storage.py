...
def store_document(client, source_name, markdown_content, upload_date, source_type='unknown', file_date=None):
    ...
    elif isinstance(markdown_content, dict) and markdown_content.get('type') == 'email':
        # preserve sender and subject metadata for direct email questions
        sender = markdown_content.get('sender', '')
        subject = markdown_content.get('subject', '')
    ...
    for i, chunk in enumerate(chunks):
        ...
        merged_meta = {
            ...
            "sender": _normalize_meta_value(meta.get('sender', '')),
            "subject": _normalize_meta_value(meta.get('subject', ''))
        }
