...
def _parse_email_headers(raw_headers):
    """Extract typical headers from email text."""
    header_map = {}
...
def convert_email_to_markdown(file_path):
    """Convert a raw email file (.eml / .txt) to structured dict with metadata."""
    ...
    parsed = _parse_email_headers(headers)
    ...
    return {
        ...
        'date': date,
        'content': markdown,
        'file_date': extract_dates_from_text(headers + '\n' + body) or date
    }
